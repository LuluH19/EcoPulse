...
## Pitch (1 phrase)
Dashboard mono-page pour le grand public soucieux de son empreinte numérique : suivre en temps réel l'intensité carbone du réseau électrique français (Electricity Maps) et simuler l'impact CO₂ de ses usages numériques et domestiques pour agir au bon moment.
...
2. Le parsing et le nettoyage du flux carbone se font UNIQUEMENT dans `scripts/deterministic/parseCarbonData.ts`. Une route d'API ne re-parse jamais à la main.
3. Toute donnée externe (réponse Electricity Maps) doit être validée par un schéma Zod (`src/lib/electricityMapsSchema.ts` en entrée, `src/lib/carbonSchema.ts` en sortie) AVANT d'être consommée. Donnée non validée = donnée refusée.
...
5. En cas de panne de l'API Electricity Maps (ou clé absente), l'application reste fonctionnelle : elle sert le fallback statique (`CARBON_FALLBACK`) en HTTP 200 avec `source: 'fallback'`. Jamais d'écran blanc, jamais de crash.
...
- `scripts/deterministic/parseCarbonData.ts` → nettoie le flux Electricity Maps brut et renvoie `[{ time, co2 }]` sur 24 h (filtre null / 0 / aberrants / hors-fenêtre)
...
- `src/lib/electricityMapsSchema.ts` → contrat Zod de la réponse brute Electricity Maps (entrée)
- `src/lib/carbonSchema.ts` → contrat Zod de `CarbonLive` (sortie) — la source de vérité des formes de données
...
- ❌ Ne JAMAIS consommer une réponse Electricity Maps sans passer par le schéma Zod.
...
- `PROJECT_RULES.md` → règles métier détaillées (seuils, formules, edge cases du flux carbone)
- `ARCHITECTURE.md` → diagramme de l'archi (composants ↔ skills ↔ scripts ↔ API Electricity Maps)
